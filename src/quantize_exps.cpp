...
void fastQuantize(const float *input, float scale, float zero_point, Index rows,
  ...
  while (Input != InputEnd) {
    ...
    int16x4_t i16x4_lo = vqmovn_s32(scaledInt_lo);
    ...
    // Vector saturating narrow integer
    // int16x4_t  vqmovn_s32(int32x4_t a);   // VQMOVN.S32 d0,q0
    int16x4_t i16x4_hi = vqmovn_s32(scaledInt_hi);

    // Combine two ints.
    // int16x8_t   vcombine_s16(int16x4_t low, int16x4_t high);
    int16x8_t i16x8 = vcombine_s16(i16x4_lo, i16x4_hi);

    // Vector saturating narrow integer
    int8x8_t i8x8 = vqmovn_s16(i16x8);
    *Output = i8x8;
    ++Output;
  };
}

void compilerSaveUsQuantize(const float *input, float scale, float zero_point,
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    output[i] = static_cast<int8_t>(scale * input[i]);
  };
}
...
#define ChooseQuantize(fn)                                                     \
  ...
  ChooseQuantize(compilerSaveUsQuantize);
  ChooseQuantize(quantize);
  return 0;
}
